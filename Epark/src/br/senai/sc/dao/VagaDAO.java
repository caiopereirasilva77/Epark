...
public class VagaDAO {
	...
	public Vaga buscarPorNumero(int id) {
		return (Vaga) em.createQuery("SELECT v FROM Vaga v WHERE v.numeroVaga = :numero")
				.setParameter("numero", id).getSingleResult();
	}

	public List<Vaga> listarVagas(String tipo, String status) {
		return em.createQuery("SELECT v FROM Vaga v WHERE v.tipoVaga =:tipo AND v.status = :status")
				.setParameter("status", status).setParameter("tipo", tipo).getResultList();
	}
}
